package com.djm.gulimall.seckill.service.ipml;
...
import com.djm.gulimall.seckill.feign.CouponFeignService;
import com.djm.gulimall.seckill.feign.ProductFeignService;
import com.djm.gulimall.seckill.interceptor.LoginUserInterceptor;
import com.djm.gulimall.seckill.service.SeckillService;
import com.djm.gulimall.seckill.to.SeckillSkuRedisTo;
import com.djm.gulimall.seckill.vo.SeckillSessionWithSkusVo;
import com.djm.gulimall.seckill.vo.SeckillSkuVo;
import com.djm.gulimall.seckill.vo.SkuInfoVo;
import lombok.Data;
...
//        try (Entry entry = SphU.entry("seckillSkus")) {
            ...
            for (String key : keys) {
                ...
                if (currentTime >= startTime && currentTime <= endTime) {
                    ...
                    BoundHashOperations<String, String, String> ss = redisTemplate.boundHashOps(SECKILL_CHARE_PREFIX);
                    List<String> listValue = ss.multiGet(range);
...
//        }
    ...
    private void saveSessionSkuInfo(List<SeckillSessionWithSkusVo> sessionData) {
        ...
        sessionData.stream().forEach(m -> {
            String key = SESSION_CACHE_PREFIX + m.getStartTime().getTime() + "_" + m.getEndTime().getTime();
            //判断Redis中是否有该信息，如果没有才进行添加
            Boolean hasKey = redisTemplate.hasKey(key);
            //缓存活动信息
            if (!hasKey) {
                //获取到活动中所有商品的skuId
                List<String> skuIds = m.getRelationSkus().stream()
    ...
    private void saveSessionInfos(List<SeckillSessionWithSkusVo> sessionData) {
        ...
        sessionData.stream().forEach(m -> {
            ...
            m.getRelationSkus().stream().forEach(seckillSkuVo -> {
                //生成随机码
                ...
                if (!operations.hasKey(redisKey)) {
                    ...
                    //3、设置当前商品的秒杀时间信息
                    redisTo.setStartTime(m.getStartTime().getTime());
                    redisTo.setEndTime(m.getEndTime().getTime());
                    //4、设置商品的随机码（防止恶意攻击）
